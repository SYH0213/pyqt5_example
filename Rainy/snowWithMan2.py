...
class SnowWidget(QWidget):
    ...
    def updateScene(self):
        ...
        if abs(self.character_x - center_x) < 2 and self.pause_timer < 150:
            self.pause_timer += 1
            if self.pause_timer % 30 == 0:
                self.breath_frames.append((self.t, self.character_x + 18, self.character_y + 5))
        elif self.pause_timer >= 150:
    ...
    def paintEvent(self, event):
        ...
        for t_created, x, y in list(self.breath_frames):
            ...
            if age > 50:
                ...
                continue
            alpha = max(0, 80 - age * 2)
            radius = 6 + age * 0.6
            painter.setBrush(QColor(255, 255, 255, alpha))
            painter.drawEllipse(QPointF(x + age * 0.4, y - age * 0.2), radius, radius * 0.4)
