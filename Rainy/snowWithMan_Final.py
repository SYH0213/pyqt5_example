...
from PyQt5.QtGui import QPainter, QColor, QPen, QBrush, QPolygon, QPainterPath
...
class SnowWidget(QWidget):
    ...
    def updateScene(self):
        ...
        if abs(self.character_x - center_x) < 2 and self.breath_count < 3:
            self.is_paused = True
            if self.t - self.last_breath_frame > 60 and not self.breath_frames:
                self.breath_frames.append((self.t, self.character_x + 18, self.character_y - 5))
                self.breath_count += 1
                self.last_breath_frame = self.t
        elif self.breath_count >= 3 and not self.breath_frames:
            self.is_paused = False
    ...
    def paintEvent(self, event):
        ...
        # 입김을 먼저 그림 (캐릭터보다 뒤에)
        for t_created, x, y in list(self.breath_frames):
            age = self.t - t_created
            if age > 90:
                self.breath_frames.remove((t_created, x, y))
                continue
            alpha = int(min(255, age * 2))  # 점점 투명해짐
            radius = (6 + age * 0.4) * 2 / 3
            painter.setBrush(QColor(255, 255, 255, max(0, 255 - alpha)))
            painter.setPen(Qt.NoPen)
            painter.drawEllipse(QPointF(x + age * 0.3, y - age * 0.15), radius, radius * 0.4)
